...
void MySummonSystem::SummonBossMonster(void)
{
	...
	{
	case 1:
		if ((GetTickCount64() - mPostTickCountForBossMonsterSummon) < 10800000)
		{
			return;
		}
		tRandom = rand();
		...
		for (index01 = MAX_MONSTER_OBJECT_BASE_5_NUM; index01 < MAX_MONSTER_OBJECT_NUM; index01++)
		{
			...
			mMONSTER_OBJECT[index01].mInvalidTimeForSummon = GetTickCount64();
			mMONSTER_OBJECT[index01].mCheckFirstLocationTime = GetTickCount64();
			...
			mMONSTER_OBJECT[index01].mCheckDetectEnemyTime = GetTickCount64();
			mMONSTER_OBJECT[index01].mCheckAttackState = FALSE;
			mMONSTER_OBJECT[index01].mTotalAttackNum = 0;
			mMONSTER_OBJECT[index01].mUpdateTimeForBroadcast = GetTickCount64();
			...
			mMONSTER_OBJECT[index01].mFirstAttackTimeForTribeSymbol = GetTickCount64();
			...
			mMONSTER_OBJECT[index01].mFirstAttackTimeForAllianceStone = GetTickCount64();
...
void MySummonSystem::SummonGuard(BOOL tCheckZone038, BOOL tCheckFirst)
{
	...
	if (!tCheckZone038)
	{
		...
		{
		case  38:
			tGuardTotalNum[0] = tGuardTribePostNum;///coordonate portals yanggok///
			tGuardMainType[0] = 6;
	...
	for (index01 = 0; index01 < 5; index01++)
	{
		...
		for (index02 = 0; index02 < tGuardTribePostNum; index02++)
		{
			...
			if (!tCheckFirst)
			{
				if (((float)(GetTickCount64() - mMONSTER_OBJECT[index03].mInvalidTimeForSummon) / 1000.0f) < (float)tMONSTER_INFO->mSummonTime[0])
				{
					index03++;
					continue;
				}
	...
	for (index01 = 0; index01 < 5; index01++)
	{
		...
		if (index02 < MAX_MONSTER_OBJECT_BASE_4_NUM)
		{
			if (tSymbolZoneNumber[index01] != mSERVER_INFO.mServerNumber)
			{
				...
				mMONSTER_OBJECT[index02].mInvalidTimeForSummon = GetTickCount64();
			}
			continue;
		}
		else
		...
		if (tSymbolSpecialType[index01] == 14 && tSymbolZoneNumber[index01] != 74)
		{
			...
			mGAME.dwAttackMonsterSymbolTime = GetTickCount64();
		}
		/** 2009. 10. 27 : Monster Symbol edit (wolf) */
...
//SUMMON_TOWER_SYSTEM
void MySummonSystem::SummonTowerSystem(BOOL IsForUpgrade, int tTowerForUpgradeIndex)
{
	int index01;
	int index02;
	int index03;
	int tSymbolSpecialType[12];
	float tSymbolLocation[12][3];
	int tTowerSort;
	unsigned int tRandom = 0;
	MONSTER_INFO* tMONSTER_INFO;

	if (IsForUpgrade == TRUE)
	{
		for(index03 = MAX_MONSTER_OBJECT_BASE_3_NUM; index03 < MAX_MONSTER_OBJECT_BASE_4_NUM; index03++)
		{
			if ((mMONSTER_OBJECT[index03].mCheckValidState) && (mMONSTER_OBJECT[index03].mMONSTER_INFO->mIndex == tTowerForUpgradeIndex - 1))
			{
				break;
			}
			if (!mMONSTER_OBJECT[index03].mCheckValidState)
		...
		tMONSTER_INFO = mMONSTER.Search(tTowerForUpgradeIndex);
		mMONSTER_OBJECT[index03].mCheckValidState = TRUE;
		mMONSTER_OBJECT[index03].mUniqueNumber = mGAME.mMonsterObjectUniqueNumber;
		mGAME.mMonsterObjectUniqueNumber++;
		mMONSTER_OBJECT[index03].mDATA.mIndex = tMONSTER_INFO->mIndex;
		...
		mMONSTER_OBJECT[index03].mDATA.mAction.aSort = 0;
		mMONSTER_OBJECT[index03].mDATA.mAction.aFrame = 0.0f;
		mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[0] = mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[0];
		mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[1] = mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[1];
		mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[2] = mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[2];
		mMONSTER_OBJECT[index03].mDATA.mAction.aTargetLocation[0] = 0.0f;
		mMONSTER_OBJECT[index03].mDATA.mAction.aTargetLocation[1] = 0.0f;
		mMONSTER_OBJECT[index03].mDATA.mAction.aTargetLocation[2] = 0.0f;
		mMONSTER_OBJECT[index03].mDATA.mAction.aFront = 0.0f;
		...
		mMONSTER_OBJECT[index03].mInvalidTimeForSummon = GetTickCount64();
		mMONSTER_OBJECT[index03].mCheckFirstLocationTime = GetTickCount64();
		mMONSTER_OBJECT[index03].mFirstLocation[0] = mMONSTER_OBJECT[index03].mFirstLocation[0];
		mMONSTER_OBJECT[index03].mFirstLocation[1] = mMONSTER_OBJECT[index03].mFirstLocation[1];
		mMONSTER_OBJECT[index03].mFirstLocation[2] = mMONSTER_OBJECT[index03].mFirstLocation[2];
		mMONSTER_OBJECT[index03].mCheckDetectEnemyTime = GetTickCount64();
		mMONSTER_OBJECT[index03].mCheckAttackState = FALSE;
		mMONSTER_OBJECT[index03].mTotalAttackNum = 0;
		mMONSTER_OBJECT[index03].mUpdateTimeForBroadcast = GetTickCount64();
	...
	else
	{
		...
		case 2:
			tTowerSort = 0;
			tSymbolSpecialType[0] = mUTIL.GetTowerSort(3, 0);
			tSymbolLocation[tTowerSort][0] = -1276.0;
			tSymbolLocation[tTowerSort][1] = -5.0f;
			tSymbolLocation[tTowerSort][2] = 1826.0f;
			break;
		case 3:
			tTowerSort = 1;
			tSymbolSpecialType[1] = mUTIL.GetTowerSort(3, 1);
			tSymbolLocation[tTowerSort][0] = -8086.0f;
			tSymbolLocation[tTowerSort][1] = 0.0f;
			tSymbolLocation[tTowerSort][2] = 6225.0f;
			break;
		case 4:
			tTowerSort = 2;
			tSymbolSpecialType[2] = mUTIL.GetTowerSort(3, 2);
			tSymbolLocation[tTowerSort][0] = 3770.0f;
			tSymbolLocation[tTowerSort][1] = 95.0f;
			tSymbolLocation[tTowerSort][2] = 3173.0f;
			break;
		case 7:
			tTowerSort = 3;
			tSymbolSpecialType[3] = mUTIL.GetTowerSort(3, 3);
			tSymbolLocation[tTowerSort][0] = -1879.0f;
			tSymbolLocation[tTowerSort][1] = 2.0f;
			tSymbolLocation[tTowerSort][2] = -1105.0f;
			break;
		case 8:
			tTowerSort = 4;
			tSymbolSpecialType[4] = mUTIL.GetTowerSort(3, 4);
			tSymbolLocation[tTowerSort][0] = 7326.0f;
			tSymbolLocation[tTowerSort][1] = 40.0f;
			tSymbolLocation[tTowerSort][2] = 4224.0f;
			break;
		case 9:
			tTowerSort = 5;
			tSymbolSpecialType[5] = mUTIL.GetTowerSort(3, 5);
			tSymbolLocation[tTowerSort][0] = -3703.0f;
			tSymbolLocation[tTowerSort][1] = -593.0f;
			tSymbolLocation[tTowerSort][2] = 6223.0f;
			break;
		case 12:
			tTowerSort = 6;
			tSymbolSpecialType[6] = mUTIL.GetTowerSort(3, 6);
			tSymbolLocation[tTowerSort][0] = -1306.0f;
			tSymbolLocation[tTowerSort][1] = -2.0f;
			tSymbolLocation[tTowerSort][2] = -380.0f;
			break;
		case 13:
			tTowerSort = 7;
			tSymbolSpecialType[7] = mUTIL.GetTowerSort(3, 7);
			tSymbolLocation[tTowerSort][0] = -7897.0f;
			tSymbolLocation[tTowerSort][1] = 9.0f;
			tSymbolLocation[tTowerSort][2] = 1899.0f;
			break;
		case 14:
			tTowerSort = 8;
			tSymbolSpecialType[8] = mUTIL.GetTowerSort(3, 8);
			tSymbolLocation[tTowerSort][0] = 6290.0f;
			tSymbolLocation[tTowerSort][1] = 340.0f;
			tSymbolLocation[tTowerSort][2] = 4775.0f;
			break;
		case 141:
			tTowerSort = 9;
			tSymbolSpecialType[9] = mUTIL.GetTowerSort(3, 9);
			tSymbolLocation[tTowerSort][0] = 4289.0f;
			tSymbolLocation[tTowerSort][1] = 0.0f;
			tSymbolLocation[tTowerSort][2] = 3645.0f;
			break;
		case 142:
			tTowerSort = 10;
			tSymbolSpecialType[10] = mUTIL.GetTowerSort(3, 10);
			tSymbolLocation[tTowerSort][0] = 32.0f;
			tSymbolLocation[tTowerSort][1] = 0.0f;
			tSymbolLocation[tTowerSort][2] = 2663.0f;
			break;
		case 143:
			tTowerSort = 11;
			tSymbolSpecialType[11] = mUTIL.GetTowerSort(3, 11);
			tSymbolLocation[tTowerSort][0] = -67.0f;
			tSymbolLocation[tTowerSort][1] = -12.0f;
			tSymbolLocation[tTowerSort][2] = 3046.0f;
			break;
		default:
			return;
		}

		tMONSTER_INFO = mMONSTER.Search(tSymbolSpecialType[tTowerSort]);
		if (tMONSTER_INFO == NULL)
		{
			return;
		}
		...
		for (index03 = MAX_MONSTER_OBJECT_BASE_3_NUM; index03 < MAX_MONSTER_OBJECT_BASE_4_NUM; index03++)
		{
			if ((mMONSTER_OBJECT[index03].mCheckValidState) && (mMONSTER_OBJECT[index03].mMONSTER_INFO->mIndex == tMONSTER_INFO->mIndex))
			{
				break;
			}
			if (!mMONSTER_OBJECT[index03].mCheckValidState)
		...
		mMONSTER_OBJECT[index03].mDATA.mAction.aSort = 0;
		mMONSTER_OBJECT[index03].mDATA.mAction.aFrame = 0.0f;
		mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[0] = tSymbolLocation[tTowerSort][0];
		mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[1] = tSymbolLocation[tTowerSort][1];
		mMONSTER_OBJECT[index03].mDATA.mAction.aLocation[2] = tSymbolLocation[tTowerSort][2];
		mMONSTER_OBJECT[index03].mDATA.mAction.aTargetLocation[0] = 0.0f;
		mMONSTER_OBJECT[index03].mDATA.mAction.aTargetLocation[1] = 0.0f;
		mMONSTER_OBJECT[index03].mDATA.mAction.aTargetLocation[2] = 0.0f;
		mMONSTER_OBJECT[index03].mDATA.mAction.aFront = 0.0f;
		...
		mMONSTER_OBJECT[index03].mInvalidTimeForSummon = GetTickCount64();
		mMONSTER_OBJECT[index03].mCheckFirstLocationTime = GetTickCount64();
		mMONSTER_OBJECT[index03].mFirstLocation[0] = tSymbolLocation[tTowerSort][0];
		mMONSTER_OBJECT[index03].mFirstLocation[1] = tSymbolLocation[tTowerSort][1];
		mMONSTER_OBJECT[index03].mFirstLocation[2] = tSymbolLocation[tTowerSort][2];
		mMONSTER_OBJECT[index03].mCheckDetectEnemyTime = GetTickCount64();
		mMONSTER_OBJECT[index03].mCheckAttackState = FALSE;
		mMONSTER_OBJECT[index03].mTotalAttackNum = 0;
		mMONSTER_OBJECT[index03].mUpdateTimeForBroadcast = GetTickCount64();
...
//SUMMON_TOWER_FOR_CONSTRUCTION_SCROLL
BOOL MySummonSystem::SummonTowerForConstruction(int tMonsterNumber)
{
	int index01;
	...
	MONSTER_INFO* tMONSTER_INFO;
	int tTowerSort;
	float tTowerLocation[12][3];

	switch (mSERVER_INFO.mServerNumber)
	{
	case 2:
		tTowerSort = 0;
		tTowerLocation[tTowerSort][0] = -1276.0f;
		tTowerLocation[tTowerSort][1] = -5.0f;
		tTowerLocation[tTowerSort][2] = 1826.0f;
		break;
	case 3:
		tTowerSort = 1;
		tTowerLocation[tTowerSort][0] = -8086.0f;
		tTowerLocation[tTowerSort][1] = 0.0f;
		tTowerLocation[tTowerSort][2] = 6225.0f;
		break;
	case 4:
		tTowerSort = 2;
		tTowerLocation[tTowerSort][0] = 3770.0f;
		tTowerLocation[tTowerSort][1] = 95.0f;
		tTowerLocation[tTowerSort][2] = 3173.0f;
		break;
	case 7:
		tTowerSort = 3;
		tTowerLocation[tTowerSort][0] = -1879.0f;
		tTowerLocation[tTowerSort][1] = 2.0f;
		tTowerLocation[tTowerSort][2] = -1105.0f;
		break;
	case 8:
		tTowerSort = 4;
		tTowerLocation[tTowerSort][0] = 7326.0f;
		tTowerLocation[tTowerSort][1] = 40.0f;
		tTowerLocation[tTowerSort][2] = 4224.0f;
		break;
	case 9:
		tTowerSort = 5;
		tTowerLocation[tTowerSort][0] = -3703.0f;
		tTowerLocation[tTowerSort][1] = -593.0f;
		tTowerLocation[tTowerSort][2] = 6223.0f;
		break;
	case 12:
		tTowerSort = 6;
		tTowerLocation[tTowerSort][0] = -1306.0f;
		tTowerLocation[tTowerSort][1] = -2.0f;
		tTowerLocation[tTowerSort][2] = -380.0f;
		break;
	case 13:
		tTowerSort = 7;
		tTowerLocation[tTowerSort][0] = -7897.0f;
		tTowerLocation[tTowerSort][1] = 9.0f;
		tTowerLocation[tTowerSort][2] = 1899.0f;
		break;
	case 14:
		tTowerSort = 8;
		tTowerLocation[tTowerSort][0] = 6290.0f;
		tTowerLocation[tTowerSort][1] = 340.0f;
		tTowerLocation[tTowerSort][2] = 4775.0f;
		break;
	case 141:
		tTowerSort = 9;
		tTowerLocation[tTowerSort][0] = 4289.0f;
		tTowerLocation[tTowerSort][1] = 0.0f;
		tTowerLocation[tTowerSort][2] = 3645.0f;
		break;
	case 142:
		tTowerSort = 10;
		tTowerLocation[tTowerSort][0] = 32.0f;
		tTowerLocation[tTowerSort][1] = 0.0f;
		tTowerLocation[tTowerSort][2] = 2663.0f;
		break;
	case 143:
		tTowerSort = 11;
		tTowerLocation[tTowerSort][0] = -67.0f;
		tTowerLocation[tTowerSort][1] = -12.0f;
		tTowerLocation[tTowerSort][2] = 3046.0f;
		break;
	}

	tPresentMonsterNum = 0;
	for (index01 = MAX_MONSTER_OBJECT_BASE_3_NUM; index01 < MAX_MONSTER_OBJECT_NUM; index01++)
	{
		if (!mMONSTER_OBJECT[index01].mCheckValidState)
		{
			continue;
	...
	mMONSTER_OBJECT[index01].mDATA.mAction.aSort = 0;
	mMONSTER_OBJECT[index01].mDATA.mAction.aFrame = 0.0f;
	mMONSTER_OBJECT[index01].mDATA.mAction.aLocation[0] = tTowerLocation[tTowerSort][0];
	mMONSTER_OBJECT[index01].mDATA.mAction.aLocation[1] = tTowerLocation[tTowerSort][1];
	mMONSTER_OBJECT[index01].mDATA.mAction.aLocation[2] = tTowerLocation[tTowerSort][2];
	mMONSTER_OBJECT[index01].mDATA.mAction.aTargetLocation[0] = 0.0f;
	mMONSTER_OBJECT[index01].mDATA.mAction.aTargetLocation[1] = 0.0f;
	mMONSTER_OBJECT[index01].mDATA.mAction.aTargetLocation[2] = 0.0f;
	tRandom = rand();
	...
	mMONSTER_OBJECT[index01].mInvalidTimeForSummon = GetTickCount64();
	mMONSTER_OBJECT[index01].mCheckFirstLocationTime = GetTickCount64();
	mMONSTER_OBJECT[index01].mFirstLocation[0] = tTowerLocation[tTowerSort][0];
	mMONSTER_OBJECT[index01].mFirstLocation[1] = tTowerLocation[tTowerSort][1];
	mMONSTER_OBJECT[index01].mFirstLocation[2] = tTowerLocation[tTowerSort][2];
	mMONSTER_OBJECT[index01].mCheckDetectEnemyTime = GetTickCount64();
	mMONSTER_OBJECT[index01].mCheckAttackState = FALSE;
	mMONSTER_OBJECT[index01].mTotalAttackNum = 0;
	mMONSTER_OBJECT[index01].mUpdateTimeForBroadcast = GetTickCount64();
